...
def chop(arr,chunk_length,padding = True):
    read_len = len(arr)
    chunks = np.split(arr,np.arange(0,read_len,chunk_length))[1:]
    length = [len(x) for x in chunks]
    if padding:
        last_chunk = chunks[-1]
        chunks[-1]= np.pad(last_chunk,(0,chunk_length-len(last_chunk)),'constant',constant_values = (0,0))
    return chunks,length

def extract(args):
    iterator = fast5_iter(args.input,mode = 'r')
    read_count = {"No basecall":0, "Succeed":0}
    ...
    signal_chunks, durations, kmer_chunks = [],[],[]
    loop_obj = tqdm(iterator)
    for read_h,signal,fast5_f,read_id in loop_obj:
        loop_obj.set_postfix_str("no entry: %d, succeed reads:%d"%(read_count["No basecall"],read_count["Succeed"]))
        try:
        ...
        curr_kmers,_ = chop(kmer_seqs,args.chunk_len)
        signal_chunks += curr_chunks
        durations += curr_duration
        kmer_chunks += curr_kmers
        read_count["Succeed"] += 1
    np.save(os.path.join(args.output,'chunks.npy'),signal_chunks)
    np.save(os.path.join(args.output,'kmers.npy'),kmer_chunks)
